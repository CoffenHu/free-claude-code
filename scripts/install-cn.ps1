...
# =============================================================================
# install-cn.ps1 — 中国国内网络环境适配版
#
# 与 install.ps1 的主要区别:
#   - Claude Code: npm install @anthropic-ai/claude-code@2.1.150
#   - Codex: npm install
#   - npm: npmmirror 镜像 (registry.npmmirror.com)
#   - uv: 优先 pip (清华镜像) 安装，备选 GitHub 代理安装脚本
#   - Python: UV_PYTHON_INSTALL_MIRROR 使用 npmmirror 镜像
#   - GitHub 资源: ghproxy.net 代理加速
# =============================================================================

# ---- 国内镜像源与代理配置 ----
$NpmRegistry = "https://registry.npmmirror.com"
$PipIndexUrl = "https://pypi.tuna.tsinghua.edu.cn/simple"
$UvPythonInstallMirror = "https://registry.npmmirror.com/-/binary/python/"
...
# ---- uv 安装脚本 (通过代理) ----
$UvInstallUrl = "${GhProxy}https://raw.githubusercontent.com/astral-sh/uv/main/scripts/install.ps1"
...
China-specific adaptions:
  - Claude Code installed via npm, version 2.1.150
  - Codex installed via npm
  - npm uses npmmirror registry (registry.npmmirror.com)
  - pip uses Tsinghua mirror (pypi.tuna.tsinghua.edu.cn)
  - Python downloads use npmmirror mirror
  - GitHub resources proxied via ghproxy.net

Prerequisites:
  - npm (bundled with Node.js; recommend nvm-windows with mirror)
  - pip (bundled with Python; or via python -m ensurepip)
"@
}
...
function Ensure-Npm {
    if (Get-ApplicationCommand "npm") {
        Write-Host "npm already found on PATH."
        return
    }
    throw "npm is not installed. Please install Node.js first (recommend nvm-windows or download from https://npmmirror.com/mirrors/node/)."
}
...
function Install-NpmPackage {
    ...
    if ($DryRun) {
        Write-Host "+ npm install -g --registry $NpmRegistry $PackageSpec"
        return
    }
    ...
    $npm = Get-ApplicationCommand "npm"
    if (-not $npm) {
        throw "npm is not available for $Name installation."
    }

    & $npm.Source install -g --registry $NpmRegistry $PackageSpec
    if ($LASTEXITCODE -ne 0) {
        throw "$Name installation via npm failed with exit code $LASTEXITCODE."
    }
}
...
function Ensure-ClaudeCode {
    if (Get-ApplicationCommand "claude") {
        ...
        Install-NpmPackage -PackageSpec "${ClaudeNpmPackage}@${ClaudeNpmVersion}" -Name "Claude Code"
        Add-KnownBinDirectories
    }
...
function Ensure-Codex {
    if (Get-ApplicationCommand "codex") {
        ...
        Install-NpmPackage -PackageSpec $CodexNpmPackage -Name "Codex"
        Add-KnownBinDirectories
    }
...
function Ensure-Uv {
    if ($DryRun) {
        if (Get-ApplicationCommand "uv") {
            Write-Host "+ uv --version"
            Write-Host "A compatible existing uv will be left unchanged; an obsolete one will be replaced."
        }
        else {
            Write-Host "uv is not installed; will try pip (Tsinghua mirror) then standalone installer."
    ...
    elseif (Get-Command pip3 -CommandType Application -ErrorAction SilentlyContinue) {
        $pipCmd = "pip3"
    }
    elseif (Get-Command python -CommandType Application -ErrorAction SilentlyContinue) {
        $pythonCmd = Get-ApplicationCommand "python"
        $checkPip = & $pythonCmd.Source -m pip --version 2>&1 | Out-String
    ...
    if ($pipCmd) {
        Write-Host "Attempting uv installation via $pipCmd (Tsinghua mirror)..."
        if ($pipCmd -eq "python") {
            $pipArgs = @("-m", "pip", "install", "-i", $PipIndexUrl, "uv")
        }
        else {
            $pipArgs = @("install", "-i", $PipIndexUrl, "uv")
        }
        Write-Host "+ $pipCmd $($pipArgs -join ' ')"
        & $pipCmd @pipArgs
        if ($LASTEXITCODE -eq 0) {
            Add-KnownBinDirectories
        }
        else {
            Write-Host "pip install failed, falling back to standalone installer via ghproxy..."
            Invoke-DownloadedPowerShellInstaller -Url $UvInstallUrl -Name "uv"
            Add-KnownBinDirectories
        }
    }
    else {
        Write-Host "pip not found, using standalone installer via ghproxy..."
        Invoke-DownloadedPowerShellInstaller -Url $UvInstallUrl -Name "uv"
        Add-KnownBinDirectories
    }
...
Write-Step "Ensuring Claude Code is installed (npm: ${ClaudeNpmPackage}@${ClaudeNpmVersion})"
Ensure-ClaudeCode

Write-Step "Ensuring Codex is installed (npm: ${CodexNpmPackage})"
Ensure-Codex

Write-Step "Ensuring Pi is installed"
Ensure-Pi

Write-Step "Ensuring uv $MinUvVersion or newer is installed (CN mirrors: pip + ghproxy)"
Ensure-Uv
